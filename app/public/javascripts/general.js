(function($, $2) {
  ...
  function pageTop() {
    var classAction = $(this).scrollTop() > 20 ? 'removeClass' : 'addClass';
    $body[classAction]('top');
  }
  ...
  $('.popup-modal').magnificPopup({
    ...
    callbacks: {
      beforeOpen() {
        const bookingForm = document.getElementById('booking-form').querySelector('form');
        const now = new Date();

        // Go to tomorrow
        now.setDate(now.getDate() + 1);
        bookingForm.elements.date.value = now.toISOString().split('T')[0];
        bookingForm.elements.time.value = now.getHours() + ':00';
      }
    }
  });
